...
func TestFindConflict(t *testing.T) {
	...
	tests := []struct {
		...
		{[]pb.Entry{{Index: 1, Term: 1}, {Index: 2, Term: 2}, {Index: 3, Term: 3},
		...
		{[]pb.Entry{{Index: 2, Term: 4}, {Index: 2, Term: 4}, {Index: 3, Term: 4}}, 2},
		{[]pb.Entry{{Index: 3, Term: 4}, {Index: 4, Term: 4}, {Index: 5, Term: 4}}, 3},
	}

	for i, tt := range tests {
		raftLog := newRaftLog(NewMemoryStorage(), raftLogger)
...
func TestIsUpToDate(t *testing.T) {
	...
	for i, tt := range tests {
		...
		if gUpToDate != tt.wUpToDate {
			t.Errorf("#%d: uptodate = %t, want %t", i, gUpToDate, tt.wUpToDate)
		}
	}
}
...
func TestAppend(t *testing.T) {
	...
	for i, tt := range tests {
		...
		raftLog := newRaftLog(storage, raftLogger)
...
func TestStableToWithSnap(t *testing.T) {
	...
	for i, tt := range tests {
		...
		raftLog := newRaftLog(s, raftLogger)
...
func TestIsOutOfBounds(t *testing.T) {
	...
	l := newRaftLog(storage, raftLogger)
