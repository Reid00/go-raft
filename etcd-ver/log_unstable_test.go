...
func TestUnstableMaybeFirstIndex(t *testing.T) {
	tests := []struct {
		...
		// no snapshot
		{
			[]pb.Entry{{Index: 5, Term: 1}}, 5, nil, false, 0,
		},
		{
			[]pb.Entry{}, 0, nil, false, 0,
		},
		// has snapshot
		{
		...
		{
			[]pb.Entry{}, 5, &pb.Snapshot{Metadata: pb.SnapshotMetadata{Index: 4, Term: 1}}, true, 5,
		},
	}
...
func TestUnstableMaybeLastIndex(t *testing.T) {
	tests := []struct {
		...
		// no snapshot
		{
			[]pb.Entry{{Index: 5, Term: 1}}, 5, nil, true, 5,
		},
		{
			[]pb.Entry{}, 0, nil, false, 0,
		},
		...
		{
			[]pb.Entry{}, 5, &pb.Snapshot{Metadata: pb.SnapshotMetadata{Index: 4, Term: 1}}, true, 4,
		},
	}
	...
	for i, tt := range tests {
		...
		idx, ok := u.maybeLastIndex()
		if ok != tt.wok {
			t.Errorf("#%d, ok = %t, want %t, ", i, ok, tt.wok)
		}

		if idx != tt.windex {
			t.Errorf("#%d, index = %d, want %d, ", i, idx, tt.windex)
		}
	}
}
...
func TestUnstableStableTo(t *testing.T) {
	tests := []struct {
		...
		{
			[]pb.Entry{{Index: 5, Term: 1}, {Index: 6, Term: 1}}, 5, nil,
			5, 2, // stable to the first entry and term mismatch
			5, 2,
		},
		{
			[]pb.Entry{{Index: 5, Term: 1}}, 5, nil,
			4, 1, // stable to the old entry
			5, 1,
		},
		{
			[]pb.Entry{{Index: 5, Term: 1}}, 5, nil,
			4, 2, // stable to the old entry
			5, 1,
		},
		// with snapshot
		{
		...
		},
		{
			...
			4, 1, // stable to snapshot and term mismatch
			5, 1,
		},
	}
