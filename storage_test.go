...
func TestStorageTerm(t *testing.T) {
	...
	for i, tt := range tests {
		...
		func() {
			...
			if term != tt.wterm {
				t.Errorf("#%d: term = %d, want %v", i, term, tt.wterm)
			}
		}()
	}
}
...
func TestStorageEntries(t *testing.T) {
	...
	// t.Errorf("entris size: %d", ents[1].Size())
	// t.Errorf("entris size: %d", ents[3].Size()/2)
	tests := []struct {
...
func TestStorageLastIndex(t *testing.T) {
	...
	s.Append([]pb.Entry{{Index: 6, Term: 6}})
...
func TestStorageFirstIndex(t *testing.T) {
	...
	if first != 4 {
		t.Errorf("first = %v, want %d", first, 4)
	}
	...
	if first != 5 {
		t.Errorf("first = %v, want %d", first, 5)
	}
}
...
func TestStorageApplySnapshot(t *testing.T) {
	...
	//Apply Snapshot fails due to ErrSnapOutOfDate (last snapshot applied since)
	i = 1
	tt = tests[i]
	err = s.ApplySnapshot(tt)
