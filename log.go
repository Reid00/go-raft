...
func (l *raftLog) maybeAppend(index, logTerm, committed uint64, ents ...pb.Entry) (lastnewi uint64, ok bool) {
	if l.matchTerm(index, logTerm) {
		...
		default:
			offset := index + 1
			// NOTE append the index after ci index
...
func (l *raftLog) append(ents ...pb.Entry) uint64 {
	...
	if after := ents[0].Index - 1; after < l.committed {
		l.logger.Panicf("after(%d) is out of range [commit(%d)]", after, l.committed)
	}
...
func (l *raftLog) term(i uint64) (uint64, error) {
	...
	if err == ErrCompacted || errors.Is(err, ErrUnavailable) {
		return 0, err
	}
	panic(err)
}
...
// allEntries returns all entreis in the log
...
func (l *raftLog) mustCheckOutOfBounds(lo, hi uint64) error {
	...
	if hi > fi+length {
		l.logger.Panicf("slice[%d, %d] out of boud [%d, %d]", lo, hi, fi, l.lastIndex())
	}
	return nil
}
